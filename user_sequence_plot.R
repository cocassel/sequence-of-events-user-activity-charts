...
# Don't read column labels of CSV
nonredundant_data = read.table("firebase_data_shining_heat_4904_nonredundant.csv", sep=",",skip = 1)
...
####################################### Call the function to plot some charts ###################################

# Print a sequence chart to console
plot_sequence_chart("-K74qVYKCfhvN0oLGP-Z")
plot_sequence_chart(session_ids[1])

# Save a sequence chart as a JPG
ggsave("example_sequence_chart1.jpg", plot = plot_sequence_chart("-KAvOqBdpZhKrbwUkKzu"), device = NULL, path = NULL,
       scale = 1, width = 20, height = 14, units = c("in", "cm", "mm"),
       dpi = 300, limitsize = FALSE)

# Save a sequence chart as a PNG
ggsave("example_sequence_chart2.png", plot = plot_sequence_chart("-K8VVFKW6JotbRw5GMNy"), device = NULL, path = NULL,
       scale = 1, width = 20, height = 14, units = c("in", "cm", "mm"),
       dpi = 300, limitsize = FALSE)


# Print all charts to console (plots tab)
for(i in 1:NROW(session_ids)) {
  plot_sequence_chart(session_ids[i])
}

# Save all charts to jpg
for(i in 1:NROW(session_ids)) {
  ggsave(paste("sequence_charts_jpg/",session_ids[i],".jpg"), plot = plot_sequence_chart(session_ids[i]), device = NULL, path = NULL,
        scale = 1, width = 20, height = 14, units = c("in", "cm", "mm"),
         dpi = 300, limitsize = FALSE)
}

# Save all charts to png
for(i in 1:NROW(session_ids)) {
  ggsave(paste("sequence_charts_png/",session_ids[i],".png"), plot = plot_sequence_chart(session_ids[i]), device = NULL, path = NULL,
        scale = 1, width = 20, height = 14, units = c("in", "cm", "mm"),
         dpi = 300, limitsize = FALSE)
}
